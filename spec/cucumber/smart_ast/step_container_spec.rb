  ...
  module SmartAst
    describe StepContainer do
      ...
      it "should include the table argument to a step" do
        rows = [%w(a b), %w(c d)]
        @step_container.steps << Step.new("Given", "a table", 2, nil)
      ...
      it "should include py_string argument to a step" do
        content = "Long string"
        @step_container.steps << Step.new("Given", "a py_string", 2, nil)
      ...
      it "should handle many steps with arguments" do
        ...
        @step_container.steps << Step.new("Given", "a normal step", 2, nil)
        @step_container.steps << Step.new("When", "a table", 3, nil)
        @step_container.table = Table.new(rows, 4)
        @step_container.steps << Step.new("Then", "a py_string", 5, nil)
        @step_container.py_string = PyString.new(content, 6)
        @step_container.steps << Step.new("And", "another normal step", 7, nil)
